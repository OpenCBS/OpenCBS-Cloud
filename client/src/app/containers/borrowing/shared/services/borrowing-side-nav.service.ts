...
@Injectable()
export class BorrowingSideNavService {

  getNavList(currentRoute, options?: NavConfigOptions) {
    const editUrlPart = (options && options.editMode) ? 'edit/' : '';
    const idUrlPart = (options && options.borrowingId) ? `${options.borrowingId}/` : '';
    ...
    if (options && options['status'] === 'ACTIVE' || options && options['status'] === 'CLOSED') {
      navs.push({
          name: 'EVENTS',

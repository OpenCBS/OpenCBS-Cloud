...
@Component({
  selector: 'cbs-loan-collateral-form',
  templateUrl: 'loan-collateral-form.component.html',
  styleUrls: ['loan-collateral-form.component.scss']
})
export class LoanCollateralFormComponent implements OnInit, OnDestroy {
  public collateralForm: FormGroup;
  public collateralTypeState: CollateralTypeListState;
  public customFields: FieldConfig[];

  private collateralTypesSub: Subscription;
  ...
  ngOnInit() {
    ...
    this.collateralTypesSub = this.store$.pipe(select(fromRoot.getCollateralTypeListState))
      .subscribe((state: CollateralTypeListState) => {

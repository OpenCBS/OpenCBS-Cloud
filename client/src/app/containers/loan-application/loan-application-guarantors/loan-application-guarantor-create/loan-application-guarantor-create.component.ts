...
@Component({
  selector: 'cbs-loan-app-guarantor-new',
  templateUrl: 'loan-application-guarantor-create.component.html',
  styleUrls: ['loan-application-guarantor-create.component.scss']
})

export class LoanAppCreateGuarantorComponent implements OnInit, OnDestroy {
  @ViewChild(GuarantorFormComponent, {static: false}) public guarantorForm: GuarantorFormComponent;
    ...
    this.createSub = this.store$.pipe(select(fromRoot.getLoanAppCreateGuarantorState))
      .subscribe((state: ILoanAppGuarantorCreate) => {
        if ( state.loaded && state.success ) {
          ...
          this.redirectToInfo(this.loanAppId, newGuarantorId);
  ...
  redirectToInfo(loanAppId, newGuarantorId) {
    this.router.navigate(['/loan-applications', loanAppId, 'guarantors', newGuarantorId]);
  }
      ...
      .subscribe((state: ILoanAppState) => {
        if ( state.success && state.loaded && state.loanApplication ) {
          ...
          this.breadcrumbLinks = [
            ...
            {
              name: 'GUARANTORS',
              link: `/loan-applications/${this.loanAppId}/guarantors`
            },
            {
              name: 'ADD',
              link: ''

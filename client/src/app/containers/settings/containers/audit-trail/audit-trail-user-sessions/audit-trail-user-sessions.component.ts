...
@Component({
  selector: 'cbs-audit-trail-user-sessions',
  templateUrl: 'audit-trail-user-sessions.component.html',
  styleUrls: ['audit-trail-user-sessions.component.scss']
})
export class AuditTrailUserSessionsComponent implements OnInit, OnDestroy {
  ...
  public reportType = 'USER_SESSIONS';
  ...
  goToPage(page: number) {
    ...
    this.router.navigate(['/audit-trail', 'user-sessions'], navigationExtras);
    this.auditTrailObjects$.dispatch(new fromStore.LoadAuditTrail(
      {
        reportType: this.reportType,
        params: {
  ...
  download() {
    ...
    setTimeout(() => {
      ...
      dataObjects.push([
        ...
        {text: 'Action'.toUpperCase(), style: 'header'}, {text: 'IP'.toUpperCase(), style: 'header'},
        {text: 'Status'.toUpperCase(), style: 'header'}
        ]);
      ...
      this.auditTrailObjects.auditTrailObjects.forEach((item) => {
            dataObjects.push([
              ...
              {text: item.loginActionType.toLowerCase(), style: 'content'},
              {text: item.ip, style: 'content'},
              {text: item.loginStatusType.toLowerCase(), style: 'content'},
            ]);
          });
          this.docDefinition = {
            content: [
      ...
      pdfMake.createPdf(this.docDefinition).download('audit-trail-sessions.pdf');
    }, 500);

...
(
    select
      ...
      olb olb
    from get_loan_schedule($1, $2) i
    where $2 >= i.start_date :: timestamp and $2::timestamp < i.maturity_date :: timestamp
)
  , late_principal as
(
  select
  ...
  from (
    select
      ...
      when -- total to pay < paid
        ...
        then
          DATE_PART('day', $2::timestamp - max(maturity_date) :: timestamp) :: integer
      else 0
      end late_days
    from (select *
          from get_loan_schedule($1, $2)

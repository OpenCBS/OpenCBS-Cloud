...
@Service
public class RequestEditGroupHandler implements RequestHandler {
    ...
    public Long approveRequest(Request request) throws Exception {
        ...
        Group group = this.groupService.getGroup(groupDto.getId());
        Group updatedGroup = this.groupService.update(this.groupMapper.zip(group, groupDto), UserHelper.getCurrentUser());
